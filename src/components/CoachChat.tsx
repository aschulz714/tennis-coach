...
export default function CoachChat() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Input */}
      <form onSubmit={handleSend} className="flex gap-2 pt-2 border-t border-neutral-800 shrink-0">
        <input
          type="text"
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder="Ask your coach..."
          className="flex-1 min-w-0 bg-neutral-800 border border-neutral-700 rounded-lg px-3 py-2 text-sm"
          disabled={loading}
        />
        <button
          type="submit"
          disabled={loading || !input.trim()}
          className="bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white px-3 py-2 rounded-lg text-sm font-medium transition shrink-0"
        >
          Send
        </button>
      </form>
